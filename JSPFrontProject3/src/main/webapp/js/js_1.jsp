...
<%--
     자바스크립트 => 기본 문법
       ...
       2) 자바스크립트에서 지원하는 객체 모델
          ...
          3. name 속성
             documnet.getElementByName("name속성명")
             <button name="btn">
          4. tag명
         ...
         ------------------- 태그 선택
         문자 조작
           textContent : 순수하게 문자열만 출력 => HTML파싱이 안 된다
           innerHTML : HTML을 파싱이 가능하다 
         스타일 조작
           태그.style.속성=값
           태그.style.color="red"
           태그.style.display='' ==> 화면 출력  ===> Jquery로 넘어가면 show
           태그.style.display='none' ==> 화면 감추기 ===> Jquery로 넘어가면 hide
         속성 조작
           img.src=""
           a.href=""
         ...
         btn.addEventListner('click',function(){}) : Jquery
 --%>
<!DOCTYPE html>
<html>
<head>
...
function gesan()
{
  ...
  avg.value=Math.round(av)    // round는 반올림 함수
    ...
    <table class="table">
     ...
     <tr>
      <td colspan="2" class="text-center">
       <input type=button id="btn" onclick="gesan()" class="btn-sm btn-primary" value="계산">
      </td>
     </tr>
    </table>
    <h1>처리 결과</h1>
