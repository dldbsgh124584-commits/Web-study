...
public class NewsServer {
	  // 접속자 저장 공간 => 모든 접속자 같은 메모리 사용 => static
	 ...
	 *  newScheduledThreadPool(1) : 예약 작업 수행하는 쓰레드 1개만 생성 
	...
	static  // 자동 호출
	{
		...
		scheduler.scheduleAtFixedRate(()->{
			try
			{
				String json=NewsRealManager.newsFindData("페이커");
				// 접속자 전체 전송
				broadcast(json);
			}catch(Exception ex) 
			{
	 ...
	 *    0초 후 실행
	 *      |
	 *    검색 : NewsRealManager.newsFindData("정일영");
	 *      |
	 *   JSON을 읽기
	 *      |
	 *   모든 접속자 전송
	...
	// Http => ws
	//      |=> handshake: HttpSession을 WebSocket에 전송
