 ...
 * 
 */
// => 스프링은 @WebServlet이 없아 : web.xml에 등록
@WebServlet("*.do")
public class DispatcherServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try
		{
			// 수정 => realPath() => 리눅스 호환
			// => 경로 / 
			String path="C:\\webDev\\webStudy\\JSPMVCProject5\\src\\main\\java";
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * 
		 *    uri="/JSPMVCProject5/food/list.do"
		 *          --------------
		 *          ContextPath
		 *    cmd="food/list.do"
		 */
		...
		try
		{
			...
			for(String cls:clsList)
			{
				...
				// 클래스안에서 메소드 찾기 시작
				// 클래스의 전체 메소드를 가지고 온다
				Method[] methods=clsName.getDeclaredMethods();   //선언된 메소드 다 가지고 와라
				for(Method m:methods)
				{
					// method 위에 @RequestMapping가 존재하는지 확인
