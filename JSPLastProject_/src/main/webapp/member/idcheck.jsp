...
$(function(){
	...
	$('#checkBtn').on('click',function(){
		...
		if(!id)
		{
			...
			$('#message').show()*/
			$('#message').text("아이디를 입력하세요").removeClass().addClass("message error").show()
			return
		}
		...
		$.ajax({
			...
			success:function(result) // result=0,1
			{
				...
				if(result==="0")
				{
					$('#message').text(id+"는(은) 사용가능한 아이디입니다").removeClass().addClass("message success").show()

					$('#okBtn').show()
				}
				else
				{
					$('#message').text(id+"는(은) 사용중인 아이디입니다").removeClass().addClass("message error").show()
					$('#okBtn').hide()
				}
			},
			error:function(err)
	...
	/*
	   ...
	   2. attr("class","message") => removeClass().addClass()
	   3. let ==> const 권장
